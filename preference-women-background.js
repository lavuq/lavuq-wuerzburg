(function(){
  const SOURCE='preference-women-wuerzburg-v2.jpg?v=20260911-1455';
  let dataUrl='';
  let loading=null;
  ...
  function load(){
    ...
    loading=fetch(SOURCE,{cache:'no-store'})
      .then(r=>{if(!r.ok) throw new Error('women image source '+r.status); return r.text();})
      .then(text=>{
        const b64=text.replace(/\s+/g,'').trim();
        if(!b64.startsWith('/9j/')) throw new Error('women image source is not jpeg base64');
