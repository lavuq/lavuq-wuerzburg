(()=>{
  ...
  const accessMember=params.get('member');
  ...
  const uniq=items=>[...new Set(items.map(clean).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'de'));
  const key=cat=>`lavuq_wue_${cat}_v2`;
  ...
  function readDemoVotes(){try{return JSON.parse(localStorage.getItem(DEMO_VOTES_KEY)||'null')||{leon:true,anna:false,sophie:false,daniel:false};}catch{return{leon:true,anna:false,sophie:false,daniel:false};}}
  function writeDemoVotes(v){try{localStorage.setItem(DEMO_VOTES_KEY,JSON.stringify(v));}catch{}}
  function readDemoProposal(){try{return JSON.parse(localStorage.getItem(DEMO_PROPOSAL_KEY)||'null')||{date:'21.09.2026, 15:00',place:'blackout',confirmed:false};}catch{return{date:'21.09.2026, 15:00',place:'blackout',confirmed:false};}}
  ...
  async function loadCategory(cat){
    ...
    const query=`[out:json][timeout:20];area["name"="Würzburg"]["boundary"="administrative"]->.a;(nwr(area.a)["name"]${cfg.query};);out tags center;`;
    try{
  ...
  function applyDemoIdentity(){
    ...
    document.querySelectorAll('#members .member').forEach(card=>{
      ...
      if(card.classList.contains('me')!==mine)card.classList.toggle('me',mine);
      const labels=[...card.querySelectorAll('.muted')].filter(x=>x.textContent.trim()==='Du');
      if(mine&&labels.length===0){const d=document.createElement('div');d.className='muted';d.textContent='Du';strong.parentElement.appendChild(d);}
      if(!mine)labels.forEach(x=>x.remove());
      if(mine&&labels.length>1)labels.slice(1).forEach(x=>x.remove());
    });
  }
  ...
  function lockFutureMeetings(){
    document.querySelectorAll('.meeting').forEach(card=>{
      ...
      if(!card.classList.contains('meeting-locked'))card.classList.add('meeting-locked');
      ...
      const schedule=card.querySelector('.schedule-panel');if(schedule&&!schedule.classList.contains('hidden'))schedule.classList.add('hidden');
    });
  ...
  function captureDemoProposal(){
    if(!isDemo)return;
    const details=document.querySelector('#schedule-1 .vote-card div[style*="margin-top"]');if(!details)return;
    const lines=details.innerText.split('\n').map(x=>x.trim()).filter(Boolean);if(lines.length<2)return;
    const current=readDemoProposal();
    const date=lines[0],place=lines.slice(1).join(' ');
    if(current.date!==date||current.place!==place){current.date=date;current.place=place;writeDemoProposal(current);}
  }
  ...
  function markOwnVote(){
    document.querySelectorAll('.vote-card').forEach(card=>{
      ...
      if(isDemo&&demoUser==='leon'){const votes=readDemoVotes();if(!votes.leon){votes.leon=true;writeDemoVotes(votes);}captureDemoProposal();}
  ...
  function patchMeetingOneSummary(){
    if(!isDemo)return;
    const card=document.getElementById('meeting-1');const head=card?.querySelector('.meeting-head');if(!head)return;
    const proposal=readDemoProposal(),votes=readDemoVotes(),count=Object.values(votes).filter(Boolean).length;
    if(!proposal.date||!proposal.place)return;
    if(count===4&&!proposal.confirmed){proposal.confirmed=true;writeDemoProposal(proposal);}
    const desired=(proposal.confirmed||count===4)
      ?`<span class="meeting-number">1</span><div><strong>Treffen 1</strong><div>${esc(proposal.date)} · ${esc(proposal.place)}</div><div class="muted">Bestätigt · verbindlich bestätigt</div></div>`
      :`<span class="meeting-number">1</span><div><strong>Treffen 1</strong><div class="muted">Terminvorschlag läuft · ${count} von 4 Zustimmungen</div></div>`;
    if(head.dataset.demoSummary!==desired){head.innerHTML=desired;head.dataset.demoSummary=desired;}
  }

  function renderSharedDemoVote(){
    if(!isDemo||demoUser==='leon')return;
    const box=document.getElementById('schedule-1');
    if(!box||box.classList.contains('hidden')||box.dataset.sharedDemo==='1'||box.querySelector('.vote-card'))return;
    if(!box.querySelector('.schedule-form'))return;
    box.dataset.sharedDemo='1';
    const votes=readDemoVotes(),proposal=readDemoProposal(),yesCount=Object.values(votes).filter(Boolean).length,already=!!votes[demoUser];
    if(proposal.confirmed||yesCount===4){
      box.innerHTML=`<h3>Terminabstimmung · Treffen 1</h3><div class="status ok"><strong>Termin verbindlich bestätigt ✓</strong><br>${esc(proposal.date)} · ${esc(proposal.place)}<br>Alle 4 Teilnehmer haben zugestimmt.</div>`;
      return;
    }
    ...
    box.querySelector('[data-demo-vote="yes"]')?.addEventListener('click',()=>{
      ...
      box.dataset.sharedDemo='';box.innerHTML='<div class="muted">Abstimmung wird aktualisiert …</div>';
      setTimeout(()=>{box.innerHTML='<div class="schedule-form"></div>';renderSharedDemoVote();patchMeetingOneSummary();},0);
