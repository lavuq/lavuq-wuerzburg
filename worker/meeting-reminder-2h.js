// LAVUQ – sichere 2h-Erinnerung vor einem Gruppentreffen.
...
export async function buildMeetingReminder2hDryRun(env,input={}){
 ...
 const due=msUntil>=1.5*3600000&&msUntil<=2.5*3600000;
 const reminder24hCompleted=meeting?.fields?.[MEETING_REMINDER_24H]===true;
 const reminder2hAlreadySent=meeting?.fields?.[MEETING_REMINDER_2H]===true;
 ...
 const ledger=parseLedger(meeting?.fields?.[MEETING_REMINDER_2H_LEDGER]);
 ...
 const ready=due&&reminder24hCompleted&&!reminder2hAlreadySent&&exactlyFourEligibleRecipients&&ambiguous.length===0;
 if(input.controlledOne===true){
   if(String(input.confirmation||"")!=="EINE_2H_ERINNERUNG_SENDEN")return{ok:false,status:409,code:"EXPLICIT_CONFIRMATION_REQUIRED"};
   if(!due)return{ok:false,status:409,code:"OUTSIDE_2H_WINDOW",hoursUntilMeeting:hoursUntil};
   if(!reminder24hCompleted)return{ok:false,status:409,code:"24H_REMINDER_NOT_COMPLETED"};
   if(reminder2hAlreadySent)return{ok:false,status:409,code:"2H_REMINDER_ALREADY_COMPLETED"};
   ...
   return{ok:true,status:200,state:"ONE_2H_TEST_REMINDER_SENT",dryRun:false,controlledOne:true,groupId,meetingRecordId:meeting.id,meetingAttempt:1,hoursUntilMeeting:hoursUntil,eligibleRecipientCount:4,emailsSent:1,otherEmailsSent:0,meetingReminderMarkedComplete:false,airtableChanged:false,piiExposedInResponse:false};
 }
 if(input.controlledAll===true){
   if(String(input.confirmation||"")!=="ALLE_2H_ERINNERUNGEN_SENDEN")return{ok:false,status:409,code:"EXPLICIT_CONFIRMATION_REQUIRED"};
   if(!due)return{ok:false,status:409,code:"OUTSIDE_2H_WINDOW",hoursUntilMeeting:hoursUntil};
   if(!reminder24hCompleted)return{ok:false,status:409,code:"24H_REMINDER_NOT_COMPLETED"};
   if(reminder2hAlreadySent)return{ok:true,status:200,state:"2H_REMINDER_ALREADY_COMPLETED",dryRun:false,controlledAll:true,groupId,emailsSent:0,alreadySentCount:4,duplicateSendPrevented:true,airtableChanged:false,piiExposedInResponse:false};
   if(!exactlyFourEligibleRecipients)return{ok:false,status:409,code:"NOT_EXACTLY_4_ELIGIBLE_RECIPIENTS",eligibleRecipientCount:recipients.length};
   ...
   for(const p of people){if(ledger.get(p.memberId)==="sent")continue;ledger.set(p.memberId,"sending");await patchMeeting(env,meeting.id,{[MEETING_REMINDER_2H_LEDGER]:serializeLedger(ledger)});changed=true;try{await sendMail(env,p.email,p.name,meeting?.fields?.[MEETING_DATE],text(meeting?.fields?.[MEETING_PLACE]));}catch(e){return{ok:false,status:502,code:"REMINDER_SEND_FAILED_REQUIRES_REVIEW",emailsSentBeforeFailure:sentNow,ambiguousRecipientCount:1,duplicateSendPrevented:true,airtableChanged:true,piiExposedInResponse:false};}ledger.set(p.memberId,"sent");await patchMeeting(env,meeting.id,{[MEETING_REMINDER_2H_LEDGER]:serializeLedger(ledger)});sentNow++;}
   ...
   await patchMeeting(env,meeting.id,{[MEETING_REMINDER_2H_LEDGER]:serializeLedger(ledger),[MEETING_REMINDER_2H]:true});
