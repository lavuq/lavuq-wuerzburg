...
const applicants = ["A", "B", "C", "D", "E", "F"];

// Kontrollierte Matrix für 6 Bewerber. Alle 15 möglichen Vierergruppen werden geprüft.
// F ist bewusst so gesetzt, dass die beste Gruppe nicht nur aus den ersten vier
// und auch nicht automatisch aus der bisherigen 5-Bewerber-Siegergruppe entsteht.
const pairScores = {
  [key("A", "B")]: 84,
  [key("A", "C")]: 86,
  [key("A", "D")]: 82,
...
assert(groups.length === 15, `Bei 6 Bewerbern müssen 15 4er-Kombinationen entstehen, erhalten: ${groups.length}`);
...
const previousWinner = evaluated.find((entry) => entry.group.join("") === "ACDE");

assert(firstFour, "A/B/C/D muss vorhanden sein.");
assert(previousWinner, "A/C/D/E muss vorhanden sein.");
assert(best.group.join("") !== "ABCD", "Beste Gruppe darf nicht automatisch die ersten vier sein.");
assert(best.average >= previousWinner.average, "Der 6-Bewerber-Vergleich darf keine bessere vorhandene Gruppe übersehen.");
...
console.log("Gruppenfinder 6.-Bewerber-Ranking-Test erfolgreich ✅");
...
console.log(`5. Bisherige 5er-Siegergruppe A/C/D/E -> Durchschnitt: ${previousWinner.average} | Schwächster Paar-Score: ${previousWinner.weakestPair}`);
console.log(`6. Beste Gruppe aus 6 Bewerbern: ${best.group.join("/")} | Durchschnitt: ${best.average} | Schwächster Paar-Score: ${best.weakestPair}`);
console.log("7. Alle 15 möglichen 4er-Kombinationen wurden verglichen: JA");
