// LAVUQ Treffen 3 – kontrollierte Terminankuendigung.
...
const MEETING_T3_ANNOUNCEMENT_LEDGER="fldhTUz1zjU8UtHbc";
...
async function sendMail(env,to,name,dateTime,place){
 ...
 const html=`<div style="font-family:Arial,sans-serif;line-height:1.6;color:#0b1f3a;max-width:620px;margin:0 auto"><h2>Hallo ${esc(name)},</h2><p>euer drittes LAVUQ-Treffen steht fest.</p><p><strong>Termin:</strong> ${esc(formatBerlin(dateTime))} Uhr<br><strong>Treffpunkt:</strong> ${esc(place)}</p><p>Bitte stimmt euch weiterhin direkt in eurer Gruppe ab. Du darfst auch zu diesem Treffen eine Begleitperson mitbringen. Ein öffentlicher Treffpunkt bleibt unsere Empfehlung.</p><p>Viele Grüße<br><strong>LAVUQ Würzburg</strong></p><p style="font-size:12px;color:#667085;margin-top:28px">LAVUQ ist kein Dating-Angebot. Im Mittelpunkt stehen Freundschaft, Austausch und gemeinsame Unternehmungen.</p></div>`;
 const r=await fetch("https://api.brevo.com/v3/smtp/email",{method:"POST",headers:{"api-key":env.BREVO_API_KEY,"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify({sender:{email:senderEmail,name:senderName},to:[{email:to,name}],subject:"Euer drittes LAVUQ-Treffen steht fest",htmlContent:html})});
 if(!r.ok)throw new Error(`BREVO_HTTP_${r.status}`);
}

export async function handleThirdMeetingAnnouncement(env,input={}){
 ...
 const dateTime=String(mf[MEETING_DATE]||"");const place=text(mf[MEETING_PLACE]);const recipientIds=parseIds(mf[MEETING_T3_RECIPIENT_IDS]);
 ...
 if(!dateTime||!Number.isFinite(new Date(dateTime).getTime())||new Date(dateTime).getTime()<=Date.now())return{ok:false,status:409,code:"THIRD_MEETING_DATE_INVALID_OR_PAST"};
 if(place.length<4)return{ok:false,status:409,code:"THIRD_MEETING_PLACE_MISSING"};
 ...
 if(valid.length!==3)return{ok:false,status:409,code:"THIRD_MEETING_RECIPIENT_FILTER_INVALID",validRecipientCount:valid.length};
 const people=[];
 ...
 const ledger=parseLedger(mf[MEETING_T3_ANNOUNCEMENT_LEDGER]);const ambiguous=people.filter(p=>ledger.get(p.memberId)==="sending");const sent=people.filter(p=>ledger.get(p.memberId)==="sent");const pending=people.filter(p=>ledger.get(p.memberId)!=="sent");
 ...
 if(mode==="controlledOne"&&String(input.confirmation||"")!=="EINE_TREFFEN_3_TERMINMAIL_SENDEN")return{ok:false,status:409,code:"EXPLICIT_CONFIRMATION_REQUIRED"};
 if(mode==="controlledAll"&&String(input.confirmation||"")!=="ALLE_TREFFEN_3_TERMINMAILS_SENDEN")return{ok:false,status:409,code:"EXPLICIT_CONFIRMATION_REQUIRED"};
 const targets=mode==="controlledOne"?pending.slice(0,1):pending;let sentNow=0;
 for(const p of targets){ledger.set(p.memberId,"sending");await patchMeeting(env,meetingRecordId,{[MEETING_T3_ANNOUNCEMENT_LEDGER]:serializeLedger(ledger)});try{await sendMail(env,p.email,p.name,dateTime,place);}catch(e){return{ok:false,status:502,code:"ANNOUNCEMENT_SEND_FAILED_REQUIRES_REVIEW",emailsSentBeforeFailure:sentNow,ambiguousRecipientCount:1,airtableChanged:true,duplicateSendPrevented:true,piiExposedInResponse:false};}ledger.set(p.memberId,"sent");await patchMeeting(env,meetingRecordId,{[MEETING_T3_ANNOUNCEMENT_LEDGER]:serializeLedger(ledger)});sentNow++;}
 const totalSent=people.filter(p=>ledger.get(p.memberId)==="sent").length;
 return{ok:true,status:200,state:totalSent===3?"THIRD_MEETING_ANNOUNCEMENT_SENT_TO_ALL":"ONE_THIRD_MEETING_ANNOUNCEMENT_SENT",dryRun:false,controlledOne:mode==="controlledOne",controlledAll:mode==="controlledAll",meetingAttempt:3,eligibleRecipientCount:3,emailsSent:sentNow,totalMarkedSent:totalSent,pendingRecipientCount:3-totalSent,excludedMembersReceivedEmail:false,airtableChanged:sentNow>0,duplicateSendPrevented:true,piiExposedInResponse:false};
}
