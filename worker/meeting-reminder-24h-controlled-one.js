// LAVUQ – kontrollierter Einzelversand einer 24h-Treffen-Erinnerung.
// Sendet genau EINE Test-Erinnerung und markiert den Termin NICHT als vollständig erinnert.
...
export async function handleMeetingReminder24hControlledOne(env,input={}){
 ...
 const meetings=await listTable(env,MEETINGS_TABLE);const candidates=meetings.filter(m=>linked(m?.fields?.[MEETING_GROUP_LINK],groupId)&&Number(m?.fields?.[MEETING_ATTEMPT]||0)===1&&text(m?.fields?.[MEETING_STATUS])==="Bestätigt");
 ...
 const meeting=candidates[0];if(meeting?.fields?.[MEETING_REMINDER_24H]===true)return{ok:false,status:409,code:"24H_REMINDER_ALREADY_COMPLETED"};
 const when=validDate(meeting?.fields?.[MEETING_DATE]);if(!when)return{ok:false,status:422,code:"MEETING_DATETIME_INVALID"};const msUntil=when.getTime()-now.getTime();const due=msUntil>=23*3600000&&msUntil<=25*3600000;if(!due)return{ok:false,status:409,code:"OUTSIDE_24H_WINDOW"};
 const members=await listTable(env,MEMBERS_TABLE);const recipients=members.filter(m=>linked(m?.fields?.[MEMBER_GROUP],groupId)&&acceptedActive(m)&&m?.fields?.[MEMBER_CONTACT_SHARED]===true);if(recipients.length!==4)return{ok:false,status:409,code:"NOT_EXACTLY_4_ELIGIBLE_RECIPIENTS",eligibleRecipientCount:recipients.length};
 const chosen=recipients[0];const applicantId=firstLink(chosen?.fields?.[MEMBER_APPLICANT]);if(!applicantId)return{ok:false,status:409,code:"APPLICANT_MISSING"};const a=await getApplicant(env,applicantId);const name=text(a?.fields?.[APPLICANT_NAME])||"LAVUQ Teilnehmer";const email=text(a?.fields?.[APPLICANT_EMAIL]);if(!email||!email.includes("@"))return{ok:false,status:409,code:"EMAIL_MISSING"};
 await sendMail(env,email,name,meeting?.fields?.[MEETING_DATE],text(meeting?.fields?.[MEETING_PLACE]));
 return{ok:true,status:200,state:"ONE_24H_TEST_REMINDER_SENT",groupId,emailsSent:1,eligibleRecipientCount:4,meetingReminderMarkedComplete:false,airtableChanged:false,piiExposedInResponse:false};
}
