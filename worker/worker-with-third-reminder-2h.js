import baseWorker from "./worker-with-third-reminder-24h.js";
...
export default{
 async fetch(request,env,ctx){
  ...
  if(url.pathname==="/gruppenfinder/third-meeting-reminder-2h"){
   ...
   try{
    ...
    const result=await handleThirdMeetingReminder2h(env,input);
    return json(result,Number(result?.status||(result?.ok?200:500)));
   }catch(e){
    console.error(e);
    return json({ok:false,error:"Treffen-3-2h-Erinnerung konnte nicht verarbeitet werden.",piiExposedInResponse:false},500);
   }
  }
