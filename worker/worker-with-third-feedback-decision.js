import baseWorker from "./worker-with-third-feedback-mail.js";
import { handleThirdMeetingFeedbackDecisionDryRun } from "./third-meeting-feedback-decision.js";
...
export default{
 async fetch(request,env,ctx){
  ...
  if(url.pathname==="/gruppenfinder/third-meeting-feedback-decision"){
   ...
   try{
    ...
    const result=await handleThirdMeetingFeedbackDecisionDryRun(env,input);
    return json(result,Number(result?.status||(result?.ok?200:500)));
   }catch(e){
    console.error(e);
    return json({ok:false,error:"Treffen-3-Feedback-Auswertung konnte nicht geprueft werden.",piiExposedInResponse:false,tokenExposedInResponse:false,linkExposedInResponse:false},500);
   }
  }
