// LAVUQ Treffen 2 – 24h-Erinnerung mit Dry-Run und geschuetztem Automatikversand.
...
export async function handleSecondMeetingReminder24hDryRun(env,input={}){
 ...
 const base={meetingAttempt:2,hoursUntilMeeting:hoursUntil,reminderWindowMatched:due,reminderAlreadyCompleted,recipientFilterCount:3,validRecipientCount:3,excludedCurrentAcceptedCount:Math.max(0,currentAccepted.length-3),alreadySentCount:alreadySent.length,pendingRecipientCount:pending.length,ambiguousRecipientCount:ambiguous.length,excludedMembersWouldReceiveReminder:false,duplicateSendPrevented:true,piiExposedInResponse:false,recipientIdsExposedInResponse:false};
 if(mode==="dryRun")return{ok:true,status:200,state:ready?"READY_FOR_SECOND_MEETING_24H_REMINDER":"NOT_READY_FOR_SECOND_MEETING_24H_REMINDER",dryRun:true,readOnly:true,...base,wouldSendEmails:ready?pending.length:0,emailsSent:0,airtableChanged:false};
 if(String(input.confirmation||"")!=="AUTOMATIC_SECOND_MEETING_24H")return{ok:false,status:409,code:"EXPLICIT_CONFIRMATION_REQUIRED"};
 if(!due)return{ok:true,status:200,state:"SECOND_MEETING_24H_NOT_DUE",dryRun:false,automatic:true,...base,emailsSent:0,airtableChanged:false};
 if(reminderAlreadyCompleted)return{ok:true,status:200,state:"SECOND_MEETING_24H_ALREADY_COMPLETED",dryRun:false,automatic:true,...base,emailsSent:0,airtableChanged:false};
