// LAVUQ Treffen 2 – sichere 24h-Erinnerungspruefung.
...
export async function handleSecondMeetingReminder24hDryRun(env,input={}){
 ...
 const msUntil=when.getTime()-asOf.getTime();const hoursUntil=Math.round((msUntil/3600000)*100)/100;const due=msUntil>=23*3600000&&msUntil<=25*3600000;
 const reminderAlreadyCompleted=mf[MEETING_REMINDER_24H]===true;
 ...
 const ledger=parseLedger(mf[MEETING_REMINDER_24H_LEDGER]);const ambiguous=validRecipientIds.filter(id=>ledger.get(id)==="sending");const alreadySent=validRecipientIds.filter(id=>ledger.get(id)==="sent");const pending=validRecipientIds.filter(id=>ledger.get(id)!=="sent");
 const ready=due&&!reminderAlreadyCompleted&&ambiguous.length===0&&pending.length>0;
 return{ok:true,status:200,state:ready?"READY_FOR_SECOND_MEETING_24H_REMINDER":"NOT_READY_FOR_SECOND_MEETING_24H_REMINDER",dryRun:true,readOnly:true,meetingAttempt:2,meetingConfirmed:true,hoursUntilMeeting:hoursUntil,reminderWindowMatched:due,reminderAlreadyCompleted,recipientFilterCount:3,validRecipientCount:3,excludedCurrentAcceptedCount:Math.max(0,currentAccepted.length-3),alreadySentCount:alreadySent.length,pendingRecipientCount:pending.length,ambiguousRecipientCount:ambiguous.length,wouldSendEmails:ready?pending.length:0,excludedMembersWouldReceiveReminder:false,emailsSent:0,airtableChanged:false,duplicateSendPrevented:true,piiExposedInResponse:false,recipientIdsExposedInResponse:false};
}
