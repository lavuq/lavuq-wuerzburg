import baseWorker from "./worker-with-feedback-mail.js";
import { handleThirdMeetingAnnouncement } from "./third-meeting-announcement.js";
...
export default{
 async fetch(request,env,ctx){
  ...
  if(url.pathname==="/gruppenfinder/third-meeting-announcement"){
   ...
   try{
    ...
    const result=await handleThirdMeetingAnnouncement(env,input);
    return json(result,Number(result?.status||(result?.ok?200:500)));
   }catch(e){
    console.error(e);
    return json({ok:false,error:"Treffen-3-Terminankuendigung konnte nicht verarbeitet werden.",piiExposedInResponse:false},500);
   }
  }
