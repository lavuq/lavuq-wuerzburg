import baseWorker from "./worker-with-third-feedback-checkin.js";
...
export default{
 async fetch(request,env,ctx){
  ...
  if(url.pathname==="/gruppenfinder/third-meeting-feedback-mail"){
   ...
   try{
    ...
    const result=await handleThirdMeetingFeedbackMailControlledOne(env,input);
    return json(result,Number(result?.status||(result?.ok?200:500)));
   }catch(e){
    console.error(e);
    return json({ok:false,error:"Treffen-3-Feedback-Testmail konnte nicht verarbeitet werden.",piiExposedInResponse:false},500);
   }
  }
