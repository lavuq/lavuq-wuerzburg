...
export default{
 async fetch(request,env,ctx){
  ...
  if(url.pathname==="/gruppenfinder/second-meeting-communication-dry-run"){
   if(request.method!=="POST")return json({ok:false,error:"Nur POST ist erlaubt."},405);
   if(!authorized(request,env))return json({ok:false,error:"Nicht autorisiert."},401);
   try{
    const input=await request.json().catch(()=>({}));
    const result=await handleSecondMeetingCommunicationDryRun(env,input);
    return json(result,Number(result?.status||(result?.ok?200:500)));
   }catch(e){
    console.error(e);
  ...
  if(url.pathname==="/gruppenfinder/second-meeting-announcement"){
   ...
   try{
    const input=await request.json().catch(()=>({}));
    const result=await handleSecondMeetingAnnouncement(env,input);
    return json(result,Number(result?.status||(result?.ok?200:500)));
   }catch(e){
    console.error(e);
  ...
  if(url.pathname==="/gruppenfinder/second-meeting-reminder-2h"){
   ...
   try{
    const input=await request.json().catch(()=>({}));
    const result=await handleSecondMeetingReminder2hDryRun(env,input);
    return json(result,Number(result?.status||(result?.ok?200:500)));
   }catch(e){
    console.error(e);
  ...
  if(url.pathname==="/gruppenfinder/second-meeting-feedback-checkin"){
   ...
   try{
    const input=await request.json().catch(()=>({}));
    const result=await handleSecondMeetingFeedbackCheckinDryRun(env,input);
    return json(result,Number(result?.status||(result?.ok?200:500)));
   }catch(e){
    console.error(e);
