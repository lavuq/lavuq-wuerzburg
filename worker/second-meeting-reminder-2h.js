// LAVUQ Treffen 2 – 2h-Erinnerung mit Dry-Run und geschuetztem Automatikversand.
...
async function sendMail(env,to,name,dateTime,place){
 ...
 const html=`<div style="font-family:Arial,sans-serif;line-height:1.6;color:#0b1f3a;max-width:620px;margin:0 auto"><h2>Hallo ${esc(name)},</h2><p>eine kurze Erinnerung: Euer zweites LAVUQ-Treffen beginnt in ungefähr zwei Stunden.</p><p><strong>Termin:</strong> ${esc(formatBerlin(dateTime))} Uhr<br><strong>Treffpunkt:</strong> ${esc(place)}</p><p>Du darfst auch zu diesem Treffen eine Begleitperson mitbringen. Wir wünschen euch eine gute gemeinsame Zeit.</p><p>Viele Grüße<br><strong>LAVUQ Würzburg</strong></p></div>`;
 const r=await fetch("https://api.brevo.com/v3/smtp/email",{method:"POST",headers:{"api-key":env.BREVO_API_KEY,"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify({sender:{email:senderEmail,name:senderName},to:[{email:to,name}],subject:"Erinnerung: Euer zweites LAVUQ-Treffen beginnt bald",htmlContent:html})});
...
export async function handleSecondMeetingReminder2hDryRun(env,input={}){
 ...
 if(attempt!==2)return{ok:false,status:409,code:"NOT_SECOND_MEETING"};if(statusName!=="Bestätigt")return{ok:false,status:409,code:"SECOND_MEETING_NOT_CONFIRMED"};if(!groupId)return{ok:false,status:409,code:"GROUP_LINK_MISSING"};if(!Number.isFinite(when.getTime()))return{ok:false,status:422,code:"MEETING_DATETIME_INVALID"};if(place.length<4)return{ok:false,status:409,code:"MEETING_PLACE_MISSING"};if(recipientIds.length!==3)return{ok:false,status:409,code:"EXPECTED_EXACTLY_3_SECOND_MEETING_RECIPIENTS",recipientFilterCount:recipientIds.length};
 ...
 const reminder24hActuallyCompleted=mf[MEETING_REMINDER_24H]===true;const reminder24hSimulated=mode==="dryRun"&&input.simulate24hCompleted===true;const reminder24hEffectiveCompleted=reminder24hActuallyCompleted||reminder24hSimulated;const reminder2hAlreadyCompleted=mf[MEETING_REMINDER_2H]===true;
 ...
 const ledger=parseLedger(mf[MEETING_REMINDER_2H_LEDGER]);const ambiguous=validRecipientIds.filter(id=>ledger.get(id)==="sending");const alreadySent=validRecipientIds.filter(id=>ledger.get(id)==="sent");const pending=validRecipientIds.filter(id=>ledger.get(id)!=="sent");const ready=due&&reminder24hEffectiveCompleted&&!reminder2hAlreadyCompleted&&ambiguous.length===0&&pending.length>0;
 const base={meetingAttempt:2,hoursUntilMeeting:hoursUntil,reminderWindowMatched:due,reminder24hActuallyCompleted,reminder24hSimulated,reminder24hEffectiveCompleted,reminder2hAlreadyCompleted,recipientFilterCount:3,validRecipientCount:3,excludedCurrentAcceptedCount:Math.max(0,currentAccepted.length-3),alreadySentCount:alreadySent.length,pendingRecipientCount:pending.length,ambiguousRecipientCount:ambiguous.length,excludedMembersWouldReceiveReminder:false,duplicateSendPrevented:true,piiExposedInResponse:false,recipientIdsExposedInResponse:false};
 if(mode==="dryRun")return{ok:true,status:200,state:ready?"READY_FOR_SECOND_MEETING_2H_REMINDER":"NOT_READY_FOR_SECOND_MEETING_2H_REMINDER",dryRun:true,readOnly:true,...base,wouldSendEmails:ready?pending.length:0,emailsSent:0,airtableChanged:false};
 if(String(input.confirmation||"")!=="AUTOMATIC_SECOND_MEETING_2H")return{ok:false,status:409,code:"EXPLICIT_CONFIRMATION_REQUIRED"};
 if(!due)return{ok:true,status:200,state:"SECOND_MEETING_2H_NOT_DUE",dryRun:false,automatic:true,...base,emailsSent:0,airtableChanged:false};
 if(!reminder24hActuallyCompleted)return{ok:true,status:200,state:"SECOND_MEETING_2H_WAITING_FOR_24H",dryRun:false,automatic:true,...base,emailsSent:0,airtableChanged:false};
 if(reminder2hAlreadyCompleted)return{ok:true,status:200,state:"SECOND_MEETING_2H_ALREADY_COMPLETED",dryRun:false,automatic:true,...base,emailsSent:0,airtableChanged:false};
 ...
 let sentNow=0;
 for(const p of people){if(ledger.get(p.memberId)==="sent")continue;ledger.set(p.memberId,"sending");await patchMeeting(env,meetingRecordId,{[MEETING_REMINDER_2H_LEDGER]:serializeLedger(ledger)});try{await sendMail(env,p.email,p.name,mf[MEETING_DATE],place);}catch(e){return{ok:false,status:502,code:"REMINDER_SEND_FAILED_REQUIRES_REVIEW",emailsSentBeforeFailure:sentNow,ambiguousRecipientCount:1,airtableChanged:true,duplicateSendPrevented:true,piiExposedInResponse:false};}ledger.set(p.memberId,"sent");await patchMeeting(env,meetingRecordId,{[MEETING_REMINDER_2H_LEDGER]:serializeLedger(ledger)});sentNow++;}
 ...
 await patchMeeting(env,meetingRecordId,{[MEETING_REMINDER_2H_LEDGER]:serializeLedger(ledger),[MEETING_REMINDER_2H]:true});
 return{ok:true,status:200,state:"SECOND_MEETING_2H_SENT_TO_ALL",dryRun:false,automatic:true,meetingAttempt:2,eligibleRecipientCount:3,emailsSent:sentNow,totalMarkedSent:3,pendingRecipientCount:0,excludedMembersReceivedEmail:false,reminderMarkedComplete:true,airtableChanged:true,duplicateSendPrevented:true,piiExposedInResponse:false};
}
