(function(){
  ...
  function loadMixedFix(){
    if(document.querySelector('script[data-mixed-fix="20260912-base64-2"]')) return;
    const s=document.createElement('script');
    s.src='preference-mixed-background.js?v=20260912-base64-2';
    s.defer=true;
    s.dataset.mixedFix='20260912-base64-2';
    document.head.appendChild(s);
  }

  function loadOpenFix(){
    if(document.querySelector('script[data-open-fix="20260912-open-sharp-crop-1"]')) return;
    const s=document.createElement('script');
    s.src='preference-open-background.js?v=20260912-open-sharp-crop-1';
    s.defer=true;
    s.dataset.openFix='20260912-open-sharp-crop-1';
    document.head.appendChild(s);
  }
  ...
  original.onload=()=>{
    ...
    loadMixedFix();
    loadOpenFix();
    [50,150,300,600,900,1500,2500,4000].forEach(ms=>setTimeout(()=>{applyPreferenceFixes();loadWomenFix();loadMixedFix();loadOpenFix();},ms));
  };
  ...
  loadMixedFix();
  loadOpenFix();
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',()=>{applyPreferenceFixes();loadWomenFix();loadMixedFix();loadOpenFix();},{once:true});
  window.addEventListener('load',()=>{applyPreferenceFixes();loadWomenFix();loadMixedFix();loadOpenFix();},{once:true});
