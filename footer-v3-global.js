(function(){
  ...
  function applyMixedPreferenceIcon(){
    const icon=document.querySelector('.preference-card--mixed .preference-card__icon');
    if(!icon) return;
    const expected='icon-gemischt-gold-64.png?v=20260910-2346';
    const img=icon.querySelector('img');
    if(!img || !img.src.includes('icon-gemischt-gold-64.png')){
  ...
  renderFooter();
  applyMixedPreferenceIcon();
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',()=>{renderFooter();applyMixedPreferenceIcon();},{once:true});
  window.addEventListener('load',applyMixedPreferenceIcon,{once:true});
  setTimeout(renderFooter,150);
  setTimeout(applyMixedPreferenceIcon,50);
  setTimeout(applyMixedPreferenceIcon,300);
  setTimeout(applyMixedPreferenceIcon,900);
  const preferenceObserver=new MutationObserver(applyMixedPreferenceIcon);
  preferenceObserver.observe(document.documentElement,{subtree:true,childList:true});
})();